...
export async function POST(request: Request) {
  try {
    ...
    switch (action) {
      ...
      case "interactions":
        return await checkInteractions(extractedData?.medicines || [])
      default:
...
async function checkInteractions(medicines: ExtractedData["medicines"]) {
  ...
  const prompt = `Analyze potential drug interactions between the following medicines:

${medicineNames}

Generate pairwise interactions between all possible pairs.
...
If no interactions exist, return [].`
  ...
  const significantInteractions = parsed.filter((i) => i.severity !== "none")

  return Response.json({ success: true, data: significantInteractions })
}
